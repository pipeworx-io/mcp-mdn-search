...
`tools/list` at `https://gateway.pipeworx.io/mdn-search/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/mdn_search_search \
  -H 'Content-Type: application/json' \
  -d '{"query":"flexbox layout"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/mdn_search_search`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "mdn-search": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-mdn-search"]
    }
  }
}
```
